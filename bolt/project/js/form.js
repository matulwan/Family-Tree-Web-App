$(document).ready(function() {
    ...
    function displayFamilyMembers(members) {
        ...
        members.forEach(function(member) {
            const photoPath = member.photo_path ? member.photo_path : 'images/profile-placeholder.png';
            const memberCard = `
                <div class="member-card" data-id="${member.id}">
                    <div class="member-photo">
                        <img src="${photoPath}" alt="${member.full_name}">
                    </div>
                    ...
                    const modalHtml = `
                        ...
                        ">
                            <div class="modal-content" style="
                                ...
                                overflow-y: auto;
                            ">
                                <h2 style="margin-bottom: 1.5rem;">${data.full_name}</h2>
                                <div style="margin-bottom: 1rem;">
                                    <strong>Gender:</strong> ${data.gender || 'Not specified'}
                                </div>
                                <div style="margin-bottom: 1rem;">
                                    <strong>State:</strong> ${data.state || 'Not specified'}
                                </div>
                                <div style="margin-bottom: 1rem;">
                                    <strong>Occupation:</strong> ${data.occupation || 'Not specified'}
                                </div>
                                <div style="margin-bottom: 1rem;">
                                    <strong>Status:</strong> ${data.status || 'Not specified'}
                                </div>
                                <div style="margin-bottom: 1rem;">
                                    <strong>Birth Date:</strong> ${data.birth_date || 'Not specified'}
                                </div>
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data) {
                    ...
                    if (data.photo_path) {
                        $('#photoPreview').attr('src', data.photo_path);
                    }
                ...
                try {
                    ...
                    if (result.success) {
                        const successMessage = $('<div>')
                            ...
                            .html('<i class="fas fa-check-circle"></i> Family member deleted successfully!');
                        $('#familyMemberForm').before(successMessage);
                        setTimeout(() => { successMessage.fadeOut(400, function() { $(this).remove(); }); }, 3000);
                        loadFamilyMembers();
                        loadRelatedToOptions();
